...
func TestErrorStatusMarkJanitor(t *testing.T) {
	...
	ctx := context.TODO()
	err = rcRollingUpgrade.finishExecution(StatusError, 3, &ctx, instance)
	g.Expect(err).NotTo(gomega.HaveOccurred())
...
func TestFinishExecutionCompleted(t *testing.T) {
	...
	err = rcRollingUpgrade.finishExecution(StatusComplete, mockNodesProcessed, &ctx, ruObj)
	g.Expect(err).To(gomega.BeNil())
...
func TestFinishExecutionError(t *testing.T) {
	...
	err = rcRollingUpgrade.finishExecution(StatusError, mockNodesProcessed, &ctx, ruObj)
	g.Expect(err).To(gomega.BeNil())
